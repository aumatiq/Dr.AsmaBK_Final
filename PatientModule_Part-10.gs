...
function createPatient(patientData) {
  ...
  ensurePreferredContactColumn_(); // Part 10: পুরনো শীটে কলাম না থাকলে এখনই যোগ করে দেবে
  ...
  // "Email" | "WhatsApp" | "Both" — না দিলে ডিফল্ট "Both"
  let preferredContact = String(patientData.preferredContact || "Both").trim();
  ...
  sheet.appendRow([
    ...
    preferredContact, // Part 10: PreferredContact
  ]);
...
function findPatientById_(patientId) {
  ...
  const prefCol = headers.indexOf("PreferredContact"); // Part 10
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idCol]).trim() === String(patientId).trim()) {
      return {
        ...
        preferredContact: prefCol > -1 ? (data[i][prefCol] || "Both") : "Both",
      };
    }
  }
  return null;
...
function reviewTestUpload(token, recordId, decision, remarksIfRejected) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idCol]).trim() === String(recordId).trim()) {
      ...
      // ── Part 10: PreferredContact অনুযায়ী Email/WhatsApp গেট করা হচ্ছে ──
      let emailSent = false;
      let whatsappLink = "";
      if (decision === "Approved") {
        ...
        if (patient) {
          ...
          // Real-time action (Doctor/Assistant তখনই ড্যাশবোর্ডে আছে) — সরাসরি
          // wa.me লিংক ফেরত দেওয়া হচ্ছে, frontend এটা auto-open করবে
