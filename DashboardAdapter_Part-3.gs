...
function saveAppointmentData(token, data) {
  ...
  let session;
  if (data.editId) {
    session = requireDoctor(token);
  } else {
    session = requireDoctorOrReceptionist(token);
  }
  ...
  if (data.editId) {
    ...
    for (let i = 1; i < sData.length; i++) {
      if (String(sData[i][idCol]).trim() === String(data.editId).trim()) {
        ...
        // ── v3.3 (Part 17): sanitize ──
        sheet.getRange(i + 1, reasonCol + 1).setValue(sanitizeUserText_(data.reason));
...
function saveDoctorProfile(token, data) {
  ...
  const profileFields = [
    ...
    "Chamber1Name", "Chamber1Address", "Chamber1Hours", "Chamber1Serial", "Chamber1MapsLink",
    "Chamber2Name", "Chamber2Address", "Chamber2Hours", "Chamber2Serial", "Chamber2MapsLink"
  ];
