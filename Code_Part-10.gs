...
function setupPatientsTab(sheet, categoriesSheet) {
  const headers = ["PatientID", "FullName", "Phone", "Age", "Gender", "Address", "RegisteredDate", "LastVisit", "Notes", "Email", "PreferredContact"];
...
// ───────────────────────── Part 10: PreferredContact কলাম মাইগ্রেশন (স্ব-সারানো) ─────────────────────────
/**
 * পুরনো Patients শীটে "PreferredContact" কলাম না থাকলে এখানেই যোগ করে দেয় —
 * সব বিদ্যমান রো-তে ডিফল্ট "Both" বসিয়ে দেয় (মানে Email+WhatsApp দুটোই চলবে,
 * আগে যেভাবে চলছিল ঠিক সেভাবেই — কারো জন্য হঠাৎ বন্ধ হয়ে যাবে না)।
 * এটা booking/reminder ফাংশনগুলোর শুরুতে defensively কল হয়, তাই ম্যানুয়ালি
 * কিছু রান করার দরকার নেই — প্রথমবার ব্যবহারেই নিজে থেকে ঠিক হয়ে যাবে।
 */
function ensurePreferredContactColumn_() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Patients");
  if (!sheet) return;

  const lastCol = sheet.getLastColumn();
  const headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
  if (headers.indexOf("PreferredContact") > -1) return; // আগে থেকেই আছে

  const newColIdx = lastCol + 1;
  sheet.getRange(1, newColIdx).setValue("PreferredContact")
    .setFontWeight("bold").setBackground("#7C3AED").setFontColor("#FFFFFF");

  const lastRow = sheet.getLastRow();
  if (lastRow > 1) {
    const defaults = [];
    for (let i = 2; i <= lastRow; i++) defaults.push(["Both"]);
    sheet.getRange(2, newColIdx, defaults.length, 1).setValues(defaults);
  }

  // Dropdown validation — ভুল বানানে ডেটা ঢুকে যাওয়া ঠেকাবে
  const rule = SpreadsheetApp.newDataValidation()
    .requireValueInList(["Email", "WhatsApp", "Both"], true)
    .setAllowInvalid(false)
    .build();
  sheet.getRange(2, newColIdx, 1000, 1).setDataValidation(rule);
}

// ───────────────────────── Part 10: এক-ক্লিকে সেটআপ (ঐচ্ছিক — auto-migration এমনিতেই হয়) ─────────────────────────
/**
 * Apps Script এডিটর থেকে ম্যানুয়ালি একবার Run করলে PreferredContact কলাম
 * আর WhatsAppQueue শীট দুটোই এখনই তৈরি/আপডেট হয়ে যাবে। নাহলেও সমস্যা নেই —
 * প্রথম booking/reminder-এর সময় এমনিতেই নিজে থেকে হয়ে যাবে।
 */
function setupPart10WhatsAppAutomation() {
  ensurePreferredContactColumn_();
  ensureWhatsAppQueueSheet_();
  Logger.log("✅ Part 10 setup complete: PreferredContact column + WhatsAppQueue sheet ready.");
}
...
//      Part 10      → PreferredContact অনুযায়ী Email/WhatsApp গেট করা +
//                      patient-facing "Confirm via WhatsApp" লিংক ফেরত দেওয়া +
//                      staff-side WhatsApp Queue-তেও একটা ব্যাকআপ এন্ট্রি রাখা
// ─────────────────────────────────────────────────────────────
function submitPublicBooking(formData) {
  try {
    ...
    ensurePreferredContactColumn_(); // পুরনো শীটে কলাম না থাকলে এখনই যোগ করে দেবে
    ...
    // "Email" | "WhatsApp" | "Both" — ফ্রন্টএন্ড না পাঠালে ডিফল্ট "Both" (আগের আচরণের সাথে সামঞ্জস্যপূর্ণ)
    var preferredContact = String(formData.preferredContact || 'Both').trim();
    if (['Email', 'WhatsApp', 'Both'].indexOf(preferredContact) === -1) preferredContact = 'Both';
    ...
    var pHeaders          = patientsData[0];
    var prefContactCol    = pHeaders.indexOf('PreferredContact'); // K, index 10 (নতুন কলাম)
    var existingPatientId = null;
    // Patients headers:
    // A=PatientID B=FullName C=Phone D=Age E=Gender F=Address
    // G=RegisteredDate H=LastVisit I=Notes J=Email K=PreferredContact
    for (var i = 1; i < patientsData.length; i++) {
      if (String(patientsData[i][2] || '').trim() === phone) {
        ...
        // PreferredContact — patient এবার যা বেছেছে সেটাই সর্বশেষ পছন্দ হিসেবে সেভ হবে
        if (prefContactCol > -1) {
          patientsSheet.getRange(i + 1, prefContactCol + 1).setValue(preferredContact);
        }
        break;
      }
    }
    ...
    if (existingPatientId) {
      ...
      patientsSheet.appendRow([
        ...
        '',              // I Notes
        email,           // J Email
        preferredContact // K PreferredContact (Part 10)
      ]);
    }
    ...
    // ── Part 10: PreferredContact অনুযায়ী চ্যানেল ঠিক করা ──
    var channels   = resolveContactChannels_(preferredContact);
    var emailSent  = false;
    var patientWhatsAppLink = ''; // real-time — patient নিজে ক্লিক করে ক্লিনিককে কনফার্ম করবে

    // ── Send confirmation email if email provided AND channel allows ──
    ...
    // ── WhatsApp: patient তখনই ব্রাউজারে আছে (real-time), তাই দুটো জিনিসই দেওয়া হচ্ছে ──
    if (channels.sendWhatsApp && phone) {
      try {
        var profile = getProfileForEmail_();
        // ১) Patient-facing বাটন: patient নিজে ক্লিনিকের WhatsApp নম্বরে ক্লিক করে কনফার্ম করবে
        patientWhatsAppLink = buildBookingConfirmPatientWhatsAppLink_(profile, fullName, patientId, appointmentId, date, timeSlot);
        // ২) Staff-side ব্যাকআপ: patient নিজে ক্লিক না করলেও Dashboard-এর WhatsApp Queue-তে
        //    থেকে যাবে, স্টাফ পরে ক্লিনিক থেকে patient-কে কনফার্মেশন পাঠাতে পারবে
        var staffMsg = buildBookingConfirmStaffMessage_(fullName, patientId, appointmentId, date, timeSlot, profile);
        addToWhatsAppQueue_(patientId, fullName, phone, 'BookingConfirm', staffMsg, appointmentId);
      } catch (waErr) {
        Logger.log('Booking WhatsApp setup failed: ' + waErr.message);
      }
    }
    ...
    return {
      ...
      emailSent:    emailSent,
      whatsappLink: patientWhatsAppLink,
...
// ───────────────────────── Part 10: Booking Confirm — Patient-facing WhatsApp লিংক (patient → clinic) ─────────────────────────
/**
 * এটা patient-এর ফোন নম্বরে যায় না — patient-এর নিজের WhatsApp খুলে
 * ক্লিনিকের নম্বরে একটা রেডি-মেড কনফার্মেশন মেসেজ দেখায়, patient শুধু
 * Send-এ ক্লিক করলেই ক্লিনিক জেনে যাবে + patient নিজের কাছে একটা রেকর্ড থাকবে।
 */
function buildBookingConfirmPatientWhatsAppLink_(profile, patientName, patientId, appointmentId, date, timeSlot) {
  var clinicNumber = String((profile && (profile.WhatsAppNumber || profile.ContactPhone)) || '').trim();
  var cleanClinicNumber = cleanPhoneForWhatsApp_(clinicNumber);
  if (!cleanClinicNumber) return '';

  var clinicName = (profile && profile.ClinicName) || 'Dr. Asma Clinic';
  var msg =
    'Hi ' + clinicName + ', I just booked an appointment on your website.\n\n' +
  ...
  return 'https://wa.me/' + cleanClinicNumber + '?text=' + encodeURIComponent(msg);
}

// ───────────────────────── Part 10: Booking Confirm — Staff-side মেসেজ (clinic → patient, Queue-এ যায়) ─────────────────────────
function buildBookingConfirmStaffMessage_(patientName, patientId, appointmentId, date, timeSlot, profile) {
  var clinicName = (profile && profile.ClinicName) || 'Dr. Asma Clinic';
  var dn = 'Dr. ' + ((profile && profile.DoctorName) || 'Asma');
  return (
    'প্রিয় ' + patientName + ',\n\n' +
    'আপনার অ্যাপয়েন্টমেন্ট কনফার্ম হয়েছে ✅\n' +
    'Patient ID: ' + patientId + '\n' +
    'Appointment ID: ' + appointmentId + '\n' +
    'তারিখ: ' + date + '\n' +
    'সময়: ' + timeSlot + '\n\n' +
    'সময়মতো আসার জন্য অনুরোধ করা হলো।\n\n' +
    'ধন্যবাদান্তে,\n' + clinicName + ' (' + dn + ')'
  );
}
...
function runDailyReminderEmails() {
  ...
  // Build patient lookup (patientId → {email, name, phone, preferredContact}) — header-based, কলামের অবস্থান বদলালেও কাজ করবে
  var ptData    = ptSheet.getDataRange().getValues();
  var ptHeaders = ptData[0];
  var nameCol   = ptHeaders.indexOf('FullName');
  var phoneCol  = ptHeaders.indexOf('Phone');
  var emailCol  = ptHeaders.indexOf('Email');
  var prefCol   = ptHeaders.indexOf('PreferredContact');
  var ptMap  = {};
  for (var i = 1; i < ptData.length; i++) {
    ptMap[ptData[i][0]] = {
      name:  ptData[i][nameCol],
      phone: ptData[i][phoneCol],
      email: emailCol > -1 ? (ptData[i][emailCol] || '') : '',
      preferredContact: prefCol > -1 ? (ptData[i][prefCol] || 'Both') : 'Both'
    };
  }

  // Scan appointments for tomorrow, status != Cancelled
  var apptData = apptSheet.getDataRange().getValues();
  var reminderCount = 0, queuedCount = 0;
  ...
  for (var r = 1; r < apptData.length; r++) {
    ...
    if (rowDateStr === tomStr && status !== 'Cancelled' && status !== 'Completed') {
      ...
      var pt        = ptMap[pid];
      var timeSlot  = (apptData[r][3] || '').toString();
      var apptId    = apptData[r][0];
      if (!pt) continue;

      var channels = resolveContactChannels_(pt.preferredContact);

      // ── Email: রোগী এখনো ব্রাউজারে নেই (এটা রাতের auto-trigger), তাই এটাই একমাত্র
      //    সরাসরি অটোমেটেড চ্যানেল — Gmail নিজে থেকে পাঠাতে পারে বলেই সম্ভব ──
      if (channels.sendEmail && pt.email) {
        try {
          sendReminderEmail(pt.email, pt.name, pid, apptId, rowDateStr, timeSlot);
          reminderCount++;
          Utilities.sleep(2000); // Avoid Gmail rate limit
        } catch (mailErr) {
          Logger.log('Reminder email failed for ' + pid + ': ' + mailErr.message);
        }
      }

      // ── WhatsApp: কেউ তখন ব্রাউজারে নেই (এটা 9AM সার্ভার ট্রিগার), তাই wa.me
      //    লিংক এখনই ওপেন করানো সম্ভব না — Dashboard-এর "Pending WhatsApp Sends"
      //    Queue-তে রাখা হচ্ছে, স্টাফ সকালে একবার রিভিউ করে পাঠিয়ে দেবে (ফ্রি) ──
      if (channels.sendWhatsApp && pt.phone) {
        try {
          var msg =
            'প্রিয় ' + pt.name + ',\n\n' +
            'আপনাকে মনে করিয়ে দেওয়া হচ্ছে — আগামীকাল আপনার অ্যাপয়েন্টমেন্ট আছে।\n' +
            'তারিখ: ' + rowDateStr + '\n' +
            'সময়: ' + timeSlot + '\n' +
            'Patient ID: ' + pid + '\n\n' +
            'সময়মতো আসার জন্য অনুরোধ করা হলো।';
          addToWhatsAppQueue_(pid, pt.name, pt.phone, 'Reminder', msg, apptId);
          queuedCount++;
        } catch (waErr) {
          Logger.log('Reminder WhatsApp queue failed for ' + pid + ': ' + waErr.message);
        }
      }
    }
  }
  Logger.log('Daily reminders for ' + tomStr + ' — Emails sent: ' + reminderCount + ', WhatsApp queued: ' + queuedCount);
}
