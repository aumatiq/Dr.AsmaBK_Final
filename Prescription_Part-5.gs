...
function savePrescription(token, data) {
  ...
  // v2.2 (Part 5): মেডিসিন লিস্ট JSON আকারেও সেভ হয় (Repeat Prescription ফিচারের জন্য —
  // যাতে ভবিষ্যতে structured ফরম্যাটে medicines ফেরত পড়া যায়, শুধু readable টেক্সট থেকে
  // পার্স করার ঝুঁকি না নিয়ে)।
  sheet.appendRow([
    prescriptionId,
    data.patientId,
    data.patientName || "",
  ...
  // ── ৫. Email পাঠানো (patient-এর email থাকলে) ──
  ...
  // ── ৬. WhatsApp লিংক তৈরি করে ফেরত পাঠানো (frontend auto-open করবে) ──
  let whatsappLink = "";
...
function setupAumatiqSheets() {
  ...
  const requiredSheets = {
    "Prescriptions": ["PrescriptionID", "PatientID", "PatientName", "Date", "Diagnosis", "Medicines", "Advice", "NextVisit", "CreatedBy", "FileLink", "MedicinesJSON"]
  };
  ...
  Object.keys(requiredSheets).forEach(function (sheetName) {
    ...
    if (sheet) {
      // v2.2 (Part 5): আগে থেকেই শীট থাকলেও দেখে নেওয়া হয় নতুন কোনো কলাম (যেমন MedicinesJSON)
      // যোগ করা লাগবে কিনা — পুরনো ডেটা অক্ষত থেকে শুধু ডান পাশে নতুন কলাম যোগ হবে।
      const existingHeaders = sheet.getRange(1, 1, 1, sheet.getLastColumn() || 1).getValues()[0];
      let addedCol = false;
      headers.forEach(function (h) {
        if (existingHeaders.indexOf(h) === -1) {
          sheet.getRange(1, sheet.getLastColumn() + 1).setValue(h)
            .setFontWeight("bold").setBackground("#0D1117").setFontColor("#F8F9FF");
          addedCol = true;
        }
      });
      if (addedCol) migratedList.push(sheetName);
      else skippedList.push(sheetName); // আগে থেকেই সম্পূর্ণ — কিছুই বদলানো হয়নি
      return;
    }
  ...
  const message =
    "✅ Setup সম্পন্ন।\n\n" +
    "নতুন তৈরি হলো: " + (createdList.length ? createdList.join(", ") : "কোনোটাই না।") + "\n" +
    "নতুন কলাম যোগ হলো (মাইগ্রেট): " + (migratedList.length ? migratedList.join(", ") : "কোনোটাই না।") + "\n" +
    "আগে থেকেই সম্পূর্ণ ছিল (স্পর্শ করা হয়নি): " + (skippedList.length ? skippedList.join(", ") : "কোনোটাই না।");
