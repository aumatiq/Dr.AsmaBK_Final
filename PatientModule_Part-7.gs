...
/**
 * Doctor Dashboard-এর "Recently Deleted" ভিউয়ের জন্য — এই ফাংশন এখন
 * সম্পূর্ণ ready; সংশ্লিষ্ট UI hook হবে Part 4 (Doctor Dashboard Core)-এ,
 * যেখানে সম্পূর্ণ nav/UI পুনর্গঠন হচ্ছে।
 */
function getDeletedPatients(token) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idx.isDeleted]).toUpperCase() === "TRUE") {
      deleted.push({
        patientId: data[i][idx.id],
        fullName: data[i][idx.name],
