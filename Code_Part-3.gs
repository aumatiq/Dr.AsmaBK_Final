  ...
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      if (pid === String(patientId).trim().toUpperCase() && pPhone === String(phone).trim()) {
        clearLoginAttemptsSheet_(lockIdentifier); // ── v3.3: সফল হলে কাউন্টার রিসেট ──
        return {
          success: true,
          patient: {
            patientId:      data[i][0],
    ...
    recordFailedLoginAttemptSheet_(lockIdentifier); // ── v3.3: ম্যাচ না হলে ভুল চেষ্টা রেকর্ড করা ──
