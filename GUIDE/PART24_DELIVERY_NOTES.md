...
## 🔧 বাগ ফিক্স আপডেট (recheck-এর পর)
প্রথম ডেলিভারিতে একটা রেসপনসিভ CSS বাগ ছিল: ভাষা বাটন (`#langToggleBtn`)
`.nav-ctas` div-এর ভেতরে বসানো হয়েছিল, যেটার CSS rule হলো
`display:none` (শুধু 1024px-এর বেশি চওড়া স্ক্রিনে `display:flex` হতো)।
মানে **1024px-এর নিচে যেকোনো স্ক্রিনে (ল্যাপটপ উইন্ডো ছোট করা থাকলে, ট্যাবলেট,
মোবাইল) বাটনটা সম্পূর্ণ অদৃশ্য ছিল** — তাই ভাষা বদলানোর কোনো উপায় চোখে
পড়েনি। এখন বাটনটা `.nav-ctas`-এর বাইরে আলাদা করে বসানো হয়েছে (হ্যামবার্গার
মেনুর ঠিক পাশে), যা **সব স্ক্রিন সাইজে সবসময় দেখা যাবে** — ডেস্কটপ, ট্যাবলেট,
মোবাইল সব জায়গায়। Playwright দিয়ে 1440px ও 390px — দুই সাইজেই টেস্ট করে
কনফার্ম করা হয়েছে যে বাটন দেখা যাচ্ছে এবং ক্লিক করলে পুরো পেজ বাংলায়
বদলে যাচ্ছে।
