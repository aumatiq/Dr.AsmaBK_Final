 ...
 * Part 3: Patient Module (PatientModule.gs) — merged with Part 2/7/17
 * ─────────────────────────────────────────────
 * Patient CRUD, Search, Test Upload (Admin + Self), Approval workflow,
 * Part 2 (name-autocomplete login + brute-force), Part 7 (soft-delete +
 * permission fix), Part 17 (input sanitization + audit log)।
 */

// ───────────────────────── BRUTE-FORCE PROTECTION CONFIG (Part 2) ─────────────────────────
...
function createPatient(patientData) {
  ...
  // ── v3.3 (Part 17 — Security Hardening): এই ফাংশন পাবলিক ওয়েবসাইট থেকে
  //    (login ছাড়াই) কল হয় বলে, ফ্রি-টেক্সট ফিল্ডগুলো থেকে HTML/script
  //    ট্যাগ স্ট্রিপ করে দেওয়া হচ্ছে — stored XSS ঠেকানোর জন্য দ্বিতীয় স্তরের সুরক্ষা ──
  sheet.appendRow([
    newId,
    sanitizeUserText_(patientData.fullName),
    patientData.phone,
    patientData.age || "",
    patientData.gender || "",
    sanitizeUserText_(patientData.address),
    today,
    "", // LastVisit — এখনো কোনো visit হয়নি
    sanitizeUserText_(patientData.notes),
...
// ───────────────────────── হেল্পার: ফোন নম্বর normalize করা (Part 2) ─────────────────────────
/**
...
function searchPatientNamesForAutocomplete(query) {
  ...
  for (let i = 1; i < data.length; i++) {
    // Part 7 — soft-deleted patient কখনো অটোকমপ্লিটেও আসবে না
    if (isRowDeleted_(data[i], headers)) continue;
...
// ───────────────────────── নতুন: BRUTE-FORCE PROTECTION HELPER (Part 2, cache-based) ─────────────────────────
// নোট: এই ফাংশনগুলো শুধু patientPortalLoginByName()-এর জন্য (CacheService,
// দ্রুত/অস্থায়ী)। পুরনো patientPortalLogin() ও Doctor/Assistant roleLogin()-এর
// জন্য Part 17-এর Security.gs-এ আলাদা নামের Sheet-based lockout ব্যবহার হয়
// (checkLoginAllowed_ / recordFailedLoginAttemptSheet_ / clearLoginAttemptsSheet_)
// — নাম আলাদা রাখা হয়েছে ইচ্ছাকৃতভাবে, যাতে একই নামের দুইটা ফাংশন Apps Script-এর
// শেয়ার্ড গ্লোবাল স্কোপে একে অপরকে override না করে।
/**
...
// ───────────────────────── নতুন: PATIENT PORTAL LOGIN (নাম + মোবাইল দিয়ে) (Part 2) ─────────────────────────
/**
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (sheetPatientId === String(patientId).trim()) {
        // Part 7 — soft-deleted patient login করতে পারবে না
        if (isRowDeleted_(data[i], headers)) {
...
function searchPatients(token, query) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Part 7 — soft-deleted patient কখনো সার্চ রেজাল্টে আসবে না
    if (isRowDeleted_(row, headers)) continue;
...
function editPatient(token, patientId, updatedFields) {
  // Part 7 — Gap ২ ফিক্স: এডিট সবসময় শুধু Doctor করতে পারবে
  // (নতুন-তৈরি পাথ createPatient() আলাদা, ওখানে requireDoctorOrReceptionist
  // থাকছে savePatientData()-এর মাধ্যমে)
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idCol]).trim() === String(patientId).trim()) {
      ...
      // ── v3.3 (Part 17): কে কোন পেশেন্টের তথ্য এডিট করলো, অডিট লগে রাখা ──
      logAuditEvent_(session, "PATIENT_EDITED", patientId, "Fields updated: " + Object.keys(updatedFields).join(", "));
...
// ───────────────────────── PATIENT SOFT-DELETE (Part 7 — নতুন, Doctor-only) ─────────────────────────
/**
 * Hard delete না — IsDeleted=TRUE, DeletedAt, DeletedBy সেট হয়। এরপর
 * এই patient getPatientsData()/searchPatients()-এ আর দেখা যাবে না,
 * কিন্তু Doctor Dashboard-এর "Recently Deleted" ভিউ থেকে যেকোনো সময়
 * restorePatient() দিয়ে ফিরিয়ে আনা যাবে।
 */
function deletePatient(token, patientId) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idCol]).trim() === String(patientId).trim()) {
      if (String(data[i][delCol]).toUpperCase() === "TRUE") {
        return { success: false, message: "এই Patient ইতিমধ্যেই ডিলিট করা আছে।" };
      }
      ...
      logAuditEvent_(session, "PATIENT_DELETED", patientId, ""); // Part 17 — অডিট লগ
      return { success: true, message: "Patient ডিলিট হয়েছে (Recently Deleted থেকে Restore করা যাবে)।" };
    }
  }
...
// ───────────────────────── PATIENT RESTORE (Part 7 — নতুন, Doctor-only) ─────────────────────────
function restorePatient(token, patientId) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idCol]).trim() === String(patientId).trim()) {
      ...
      logAuditEvent_(session, "PATIENT_RESTORED", patientId, ""); // Part 17 — অডিট লগ
      return { success: true, message: "Patient পুনরায় ফিরিয়ে আনা হয়েছে।" };
    }
  }
...
// ───────────────────────── RECENTLY DELETED PATIENTS লিস্ট (Part 7 — নতুন, Doctor-only) ─────────────────────────
/**
 * Doctor Dashboard-এর "Recently Deleted" ভিউয়ের জন্য — Part 3/4-এর UI hook করবে।
 */
function getDeletedPatients(token) {
  requireDoctor(token);
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][delCol]).toUpperCase() === "TRUE") {
      deleted.push({
        patientId: data[i][idCol],
        fullName: data[i][nameCol],
        deletedAt: data[i][delAtCol],
        deletedBy: data[i][delByCol],
      });
    }
  }

  return { success: true, deletedPatients: deleted, count: deleted.length };
}
...
function reviewTestUpload(token, recordId, decision, remarksIfRejected) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idCol]).trim() === String(recordId).trim()) {
      ...
      // ── v3.3 (Part 17): টেস্ট রেজাল্ট Approve/Reject — অডিট লগে রাখা ──
      logAuditEvent_(session, "TEST_RECORD_" + decision.toUpperCase(), recordId, "Patient: " + patientId);
